class StrengthsController < ApplicationController
  before_action :set_strength, only: %i[ edit update destroy ]
  ...
  def create
    ...
    respond_to do |format|
      if @strength.save
        format.html { redirect_to strengths_url, notice: "Strength was successfully created." }
  ...
  def update
    respond_to do |format|
      if @strength.update(strength_params)
        format.html { redirect_to strengths_url, notice: "Strength was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to strengths_path, notice: "Strength was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_strength
      @strength = Strength.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def strength_params
      params.expect(strength: [ :name, :domain ])
    end
end
